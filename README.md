...
- Diagnostics sheet with selected-TV/session/discovery details, protocol timestamps, volume/text-field status, session-scoped protocol observations, a Stage 2 protocol evidence capture with per-question status and observation provenance, copyable output, a guided validation runner, per-TV validation reports, and a persisted physical-device checklist. Protocol evidence is diagnostic context, not validation evidence; a stored validation report or explicit user/device evidence is the source for wording such as "validated on physical Google TV as of <date>" for the areas that passed.
...
`Docs/Observability.md`. Protocol evidence values such as IME app package names,
device-info strings, and feature masks stay local to Diagnostics and validation
reports; they are not sent to PostHog.
